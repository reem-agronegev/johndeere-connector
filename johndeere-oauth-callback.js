  ...
  handleDeereRoute("BigQuery export", async (req, res) => {
    ...
    for (const f of fields) {
        ...
        try {
          ...
          for (const b of bResp.values) {
            ...
            rows.dim_polygon.push({
              ...
              geom_wkt: geometryToWktRounded(geom),
              geometry_hash: geometryHash(geom),
              centroid_wkt: b.centroid ? `POINT(${num(b.centroid.lon)} ${num(b.centroid.lat)})` : null,
      ...
      for (const op of opsResult.operations) {
        ...
        rows.fact_operation.push({
          ...
          geom_wkt: built.type === "Feature" ? geometryToWktRounded(built.geometry) : null,
    ...
    archive.append(
      [
        ...
        ``,
        `Notes`,
        `  Geometry is WKT text in geom_wkt, rounded to 6 decimal places.`,
        `  It loads as STRING, not GEOGRAPHY: Deere returns inconsistent ring`,
        `  orientation and nested rings labelled as separate shells, which a`,
        `  direct GEOGRAPHY load rejects. Query core.v_polygon and`,
        `  core.v_operation, which convert with make_valid => TRUE.`,
        ``,
